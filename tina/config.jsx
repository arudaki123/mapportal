...
import { defineConfig, TextField, LocalAuthProvider, ReferenceField } from "tinacms";
    ...
    <p className="rounded-lg border shadow px-4 py-2.5 bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-200 mb-4">
      <div className="flex items-center gap-2">
        ...
        <div className={`flex-1 text-sm text-yellow-700 whitespace-normal	`}>
          To see settings changes reflected on your site, restart the T4CH CLI
  ...
  fields: [
    {
      ...
      hooks: {
        beforeSave: (values) => {
          if (!values.title) {
            values.title = values._sys.filename
              .replace(/-/g, " ") // Thay dấu "-" bằng khoảng trắng
              .replace(/\..+$/, ""); // Bỏ phần đuôi file (.md, .json,...)
          }
        },
      },
    ui: {
      itemProps: (item) => ({
        label: item.title || item._sys.filename, // Dùng title, nếu không có thì lấy tên file
      }),
    },
    },
...
const HomepageCollection = {
  ...
  label: "Homepage",
  description:
    "To see settings changes reflected on your site, you must restart the T4CH CLI after saving changes (local development only).",
...
export default defineConfig({
  ...
  schema: {
    collections: [
      ...
      //   format: "json",
      //   fields: [
      //     { name: "data", label: "Dữ liệu", type: "object",
      //     list: true,
      //     templates: [
      //       HeroBlockTemplate,
      //       FeaturesBlockTemplate,
